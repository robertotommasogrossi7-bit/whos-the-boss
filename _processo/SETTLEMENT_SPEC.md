...
>
> ⚠️ **Stato 2026-05-31**: questo descrive la chiusura **attuale (in `main`)**. La
> feature **poker-live** (`USCITA_CASH_SPEC` + `TAVOLO_LIVE_SPEC`) trasformerà la
> chiusura "alla cieca" in una **review pre-compilata** dai movimenti segnati dal
> vivo: **stessa math** (il §8/§14 resta valido — `saldoUscita = fiche − mancante`
> ne è una riformulazione), cambiano **UI e momento**. Aggiornare questo file in
> fase implementativa, **motivando ogni test che cambia** (niente regressioni mute).
